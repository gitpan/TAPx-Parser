...
BEGIN {
    $STREAMED = 'TAPx::Parser::Streamed';
    $ITER     = 'TAPx::Parser::Iterator';
    use_ok $STREAMED or die;
    use_ok $ITER     or die;
}
my $ITER_FH    = "${ITER}::FH";
...
can_ok $parser, 'results';
is $parser->results->as_string, '1..5',
  '... and the plan should parse correctly';
is $parser->results->as_string, 'ok 1 - input file opened',
  '... and the first test should parse correctly';
is $parser->results->as_string, '... this is junk',
  '... and junk should parse correctly';
is $parser->results->as_string,
  'not ok 2 first line of the input valid # TODO some data',
  '... and the second test should parse correctly';
is $parser->results->as_string, '# this is a comment',
  '... and comments should parse correctly';
is $parser->results->as_string, 'ok 3 - read the rest of the file',
  '... and the third test should parse correctly';
is $parser->results->as_string, 'not ok 4 - this is a real failure',
  '... and the fourth test should parse correctly';
is $parser->results->as_string, 'ok 5 # SKIP we have no description',
